...
/**
 * Public, unauthenticated tenant-onboarding activation flow — the
 * counterpart to AcceptInvitePage, but for a brand-new tenant's first
 * administrator (provisioned by a Platform Admin via "New Tenant") rather
 * than an existing tenant inviting an additional member. No Keycloak
 * account exists until this form actually submits — see
 * TenantActivationService on the backend for why that's deliberate.
 */
export default function ActivateTenantPage() {
  ...
  useEffect(() => {
    if (!token) {
      setPreviewError('This activation link is missing its token.')
      setLoadingPreview(false)
      return
    }
    apiClient
      .get<ActivationPreview>(`/api/platform/activate/${token}`)
      .then((res) => setPreview(res.data))
      .catch((err) => setPreviewError(err?.message || 'This activation link is invalid or has expired.'))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      await apiClient.post(`/api/platform/activate/${token}`, { password })
      setDone(true)
    } catch (err: any) {
