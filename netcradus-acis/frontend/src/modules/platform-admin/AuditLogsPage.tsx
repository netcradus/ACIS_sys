    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between border-b border-fire-border pb-4">
        <div>
          ...
          <p className="text-small text-text-muted mt-1">{totalElements} total records</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={fetchLogs} className="flex items-center gap-1.5 bg-surface-3 hover:bg-surface-2 border border-fire-border text-text-secondary font-semibold px-3 py-2 rounded-lg text-small">
            <RefreshCw className="w-3.5 h-3.5" /> Refresh
          </button>
          <button onClick={handleExportCsv} className="flex items-center gap-1.5 bg-accent-pa hover:bg-accent-pa-dark text-white font-semibold px-3 py-2 rounded-lg text-small">
            <Download className="w-3.5 h-3.5" /> Export CSV
          </button>
          <button onClick={handleExportXlsx} className="flex items-center gap-1.5 bg-accent-pa hover:bg-accent-pa-dark text-white font-semibold px-3 py-2 rounded-lg text-small">
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
        <div className="relative col-span-2">
          ...
          <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(0) }}
            ...
            className="w-full bg-surface-2 border border-fire-border rounded-lg pl-9 pr-3 py-2 text-small text-text-primary focus:outline-none focus:border-accent-pa" />
        </div>
        <select value={tenantFilter} onChange={(e) => { setTenantFilter(e.target.value); setPage(0) }}
          className="bg-surface-2 border border-fire-border rounded-lg px-3 py-2 text-small text-text-primary cursor-pointer">
        ...
        <input type="date" value={startDate} onChange={(e) => { setStartDate(e.target.value); setPage(0) }}
          className="bg-surface-2 border border-fire-border rounded-lg px-3 py-2 text-small text-text-primary" />
      ...
      <div className="ag-theme-platform-admin w-full" style={{ height: 520 }}>
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3">
            ...
            <p className="text-small text-text-muted">No audit events found.</p>
          </div>
        ) : (
          <AgGridReact
            rowData={events}
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <div className="flex gap-2">
            <button disabled={page === 0} onClick={() => setPage(p => p - 1)}
              className="px-3 py-1.5 rounded-lg text-small font-semibold bg-surface-3 border border-fire-border text-text-secondary disabled:opacity-30">
              Previous
            </button>
...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex gap-3">
      <span className="text-label text-text-muted uppercase w-28 flex-shrink-0">{label}</span>
