  ...
  return (
    <div className="space-y-6 animate-fade-in flex flex-col h-full bg-background text-text-secondary p-6 min-h-screen">
      ...
      <div className="flex items-center justify-between border-b border-fire-border pb-4">
        <h1 className="text-xl font-bold text-text-primary tracking-tight uppercase">Endpoints & Network — Self-Healing</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {[
          ...
          <div key={i} className={clsx("bg-surface-2 border border-fire-border rounded-lg p-4 flex flex-col justify-between h-24 shadow-sm", stat.border)}>
            <span className="text-2xl font-bold text-text-primary tracking-tight leading-none tabular-nums">{stat.value}</span>
            <span className="text-[9px] text-text-muted font-semibold tracking-wider uppercase mt-2">{stat.label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="bg-surface-2 border border-fire-border rounded-xl p-5 shadow-sm space-y-4">
        ...
        <div className="flex items-center justify-between border-b border-fire-border pb-3">
          <div>
            <h2 className="text-sm font-bold text-text-primary tracking-tight leading-none uppercase">Self-Healing Status</h2>
            <p className="text-[10px] text-text-muted mt-1 uppercase tracking-wider">Auto-isolation • rollback • policy drift repair</p>
          </div>
          <button 
            onClick={handleRollbackAll}
            className="bg-accent hover:bg-accent-dark text-white font-bold px-4 py-2 rounded-xl text-xs transition-colors focus:outline-none"
          >
            Rollback All Pending
          </button>
        </div>
              ...
              {filteredEndpoints.map(ep => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-4 px-4 font-bold">
                      <span className={clsx(
                        isIsolated ? "text-red-500" : "text-text-muted"
                      )}>
                        {isIsolated ? 'Yes' : 'No'}
                      </span>
                    </td>
                    <td className="py-4 px-4 font-mono font-bold">
                      <span className={clsx(
                        rollback === 'Snapshot' ? "text-orange-400" : "text-text-muted"
                      )}>
                        {rollback}
                      </span>
                    </td>
                    <td className="py-4 px-4 text-text-secondary font-mono font-semibold">
                      {version}
                    </td>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {isIsolated ? (
                          ...
                          <button 
                            ...
                            className="border border-fire-border bg-surface-3/40 hover:bg-surface-3 text-text-secondary font-bold px-3 py-1.5 rounded-lg text-[10px] uppercase tracking-wider transition-colors focus:outline-none"
                          >
                            Update
                          </button>
                        ) : (
