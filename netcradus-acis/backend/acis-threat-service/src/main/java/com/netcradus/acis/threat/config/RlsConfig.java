...
public class RlsConfig {
    ...
    /** ThreatIntelligenceService.initMockData() runs via @PostConstruct, before context refresh
     *  completes — i.e. before this CommandLineRunner — so seed inserts still happen pre-RLS. */
    @Bean
    @Order(1000)
    public CommandLineRunner enableRowLevelSecurity(JdbcTemplate jdbcTemplate) {
        return args -> RlsBootstrapper.apply(jdbcTemplate, "threat_indicators");
    }
}
