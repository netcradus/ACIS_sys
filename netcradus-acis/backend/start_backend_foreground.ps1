...
foreach ($s in $services) {
    ...
    if (Test-Path $jarFullPath) {
        ...
        $p = Start-Process java -NoNewWindow -WorkingDirectory $backendRoot -ArgumentList "-Xms128m", "-Xmx320m", "-XX:MaxMetaspaceSize=160m", "-XX:TieredStopAtLevel=1", "-jar", "`"$jarFullPath`"" -RedirectStandardOutput $outLog -RedirectStandardError $errLog -PassThru
        $processes += $p
